...
from demo.drex.train_drex_model import TrexRewardModelAD
import torch

# trex_reward_folder = trex_path + '/reward_model'
# trex_reward_file = trex_reward_folder + '/reward_ckpt_sigmoid_drex_20stateold'
# assert os.path.exists(trex_reward_file)

metadrive_basic_config = dict(
    exp_name='drex_ppo_train',
    reward_model_path = 'reward_model/reward_ckpt_sigmoid_drex_20stateold',
    env=dict(
        metadrive=dict(
...
def main(cfg):
    ...
    reward_model.reward_model.load_state_dict(torch.load(cfg['reward_model_path']))
    ...
    while True:
        if evaluator.should_eval(learner.train_iter):
            stop, rate = evaluator.eval_verbose(learner.save_checkpoint, learner.train_iter, collector.envstep)
            if stop:
                break
        # Sampling data from environments
        new_data = collector.collect(cfg.policy.collect.n_sample, train_iter=learner.train_iter)
        new_data = reward_model.estimate(new_data)
